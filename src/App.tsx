...
export default function App() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'odoo' | 'sot') => {
    const file = e.target.files?.[0];
    if (file) {
      if (type === 'odoo') setOdooFile(file);
      else setSotFile(file);
      setError(null);
      setSuccess(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!odooFile || !sotFile) {
      setError("Harap pilih kedua file (Odoo & SOT) terlebih dahulu.");
      return;
    }
    ...
    formData.append('sotFile', sotFile);
    ...
    <div className="min-h-screen bg-neutral-50 font-sans text-neutral-900 selection:bg-blue-100 selection:text-blue-900">
      <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <header className="mb-12 text-center">
          ...
          >
            Automasi penggabungan data tarikan Odoo dengan data SOT untuk persiapan import kembali ke Odoo.
          </motion.p>
        </header>
        ...
        <main className="grid gap-8 lg:grid-cols-2">
          ...
          <section className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-neutral-200/50">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-2">
                  2. Data SOT Terbaru (Template B)
                </label>
                <div 
                  onClick={() => sotInputRef.current?.click()}
                  className={`group relative flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed p-6 transition-all ${
                    sotFile ? 'border-blue-200 bg-blue-50' : 'border-neutral-200 hover:border-blue-400 hover:bg-neutral-50'
                  }`}
                >
                  <input 
                    type="file" 
                    ref={sotInputRef} 
                    onChange={(e) => handleFileChange(e, 'sot')}
                    accept=".xlsx,.xls" 
                    className="hidden" 
                  />
                  {sotFile ? (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-sm font-medium text-blue-900 truncate max-w-[150px]">{sotFile.name}</p>
                        <p className="text-xs text-blue-600">{(sotFile.size / 1024).toFixed(1)} KB</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      <FileText className="mb-2 h-8 w-8 text-neutral-400 group-hover:text-blue-500 transition-colors" />
                      <p className="text-sm text-neutral-500">Pilih file Excel SOT</p>
                    </>
                  )}
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={!odooFile || !sotFile || isProcessing}
          ...
          <section className="flex flex-col gap-6">
            <div className="rounded-2xl bg-neutral-900 p-8 text-neutral-100 shadow-xl">
              ...
              <ul className="space-y-4 text-sm text-neutral-400">
                <li className="flex gap-3">
                  ...
                  <p>Siapkan file dari <span className="text-white font-medium">Odoo</span> (Template A) dan file <span className="text-white font-medium">SOT</span> (Template B).</p>
                </li>
                <li className="flex gap-3">
            ...
            <div className="rounded-2xl border border-neutral-200 bg-white p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-xs border-b border-neutral-100 pb-2">
                  <span className="text-neutral-500">SOT Sheets</span>
